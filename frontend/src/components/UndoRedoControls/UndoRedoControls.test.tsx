...
describe('UndoRedoControls', () => {
  ...
  it('does not call onUndo when undo button is disabled and clicked', () => {
    const onUndo = vi.fn();
    render(<UndoRedoControls {...defaultProps} canUndo={false} onUndo={onUndo} />);
  ...
  it('does not call onRedo when redo button is disabled and clicked', () => {
    const onRedo = vi.fn();
    render(<UndoRedoControls {...defaultProps} canRedo={false} onRedo={onRedo} />);
  ...
  it('renders history button when onShowHistory prop is provided', () => {
    ...
    render(<UndoRedoControls {...defaultProps} onShowHistory={onShowHistory} />);

    expect(screen.getByRole('button', { name: 'Show version history' })).toBeInTheDocument();
  ...
  it('does not render history button when onShowHistory prop is not provided', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Show version history' })).not.toBeInTheDocument();
  ...
  it('renders separator when history button is present', () => {
    const onShowHistory = vi.fn();
    const { container } = render(<UndoRedoControls {...defaultProps} onShowHistory={onShowHistory} />);
  ...
  it('applies custom className', () => {
    const { container } = render(<UndoRedoControls {...defaultProps} className="custom-class" />);

    expect(container.firstChild).toHaveClass('undo-redo-controls', 'custom-class');
  });
  ...
  it('has proper button titles', () => {
    ...
    expect(screen.getByRole('button', { name: 'Undo last action' })).toHaveAttribute('title', 'Undo (Ctrl+Z)');
    expect(screen.getByRole('button', { name: 'Redo last action' })).toHaveAttribute('title', 'Redo (Ctrl+Y)');
  });
  ...
  it('has proper history button title when present', () => {
    ...
    expect(screen.getByRole('button', { name: 'Show version history' })).toHaveAttribute('title', 'Show version history');
  });
  ...
  describe('Keyboard shortcuts', () => {
    ...
    it('handles Ctrl+Y for redo', () => {
      const onRedo = vi.fn();
      const { container } = render(<UndoRedoControls {...defaultProps} onRedo={onRedo} />);
    ...
    it('does not trigger undo when canUndo is false', () => {
      const onUndo = vi.fn();
      const { container } = render(<UndoRedoControls {...defaultProps} canUndo={false} onUndo={onUndo} />);
    ...
    it('does not trigger redo when canRedo is false', () => {
      const onRedo = vi.fn();
      const { container } = render(<UndoRedoControls {...defaultProps} canRedo={false} onRedo={onRedo} />);
    ...
    it('ignores other key combinations', () => {
      const onUndo = vi.fn();
      const onRedo = vi.fn();
      const { container } = render(<UndoRedoControls {...defaultProps} onUndo={onUndo} onRedo={onRedo} />);
  ...
  describe('SVG icons', () => {
    ...
    it('renders history icon SVG when history button is present', () => {
      ...
      const historyButton = screen.getByRole('button', { name: 'Show version history' });
  ...
  describe('Edge cases', () => {
    it('handles both canUndo and canRedo being false', () => {
      render(<UndoRedoControls {...defaultProps} canUndo={false} canRedo={false} />);

      expect(screen.getByRole('button', { name: 'Undo last action' })).toBeDisabled();
      expect(screen.getByRole('button', { name: 'Redo last action' })).toBeDisabled();
    });

    it('handles empty className prop', () => {
      const { container } = render(<UndoRedoControls {...defaultProps} className="" />);

      expect(container.firstChild).toHaveClass('undo-redo-controls');
    });

    it('handles undefined className prop', () => {
      const { container } = render(<UndoRedoControls {...defaultProps} className={undefined} />);
