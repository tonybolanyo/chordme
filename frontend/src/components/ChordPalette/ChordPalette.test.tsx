...
describe('ChordPalette', () => {
  it('renders chord library title', () => {
    ...
    expect(
      screen.getByText('Click to insert chord or drag to position')
    ).toBeInTheDocument();
  });
  ...
  it('displays chord buttons', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Insert C chord/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Insert G chord/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Insert Am chord/i })
    ).toBeInTheDocument();
  });
  ...
  it('calls onChordSelect when chord button is clicked', () => {
    ...
    const cChordButton = screen.getByRole('button', {
      name: /Insert C chord/i,
    });
  ...
  it('filters chords by search term', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Insert Cmaj7 chord/i })
    ).toBeInTheDocument();

    // Should not show regular major chords
    expect(
      screen.queryByRole('button', { name: /Insert C chord/i })
  ...
  it('filters chords by category', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Insert Em chord/i })
    ).toBeInTheDocument();
  ...
  it('shows empty state when no chords match search', () => {
    ...
    expect(
      screen.getByText(/No chords found matching "XYZ"/i)
    ).toBeInTheDocument();
