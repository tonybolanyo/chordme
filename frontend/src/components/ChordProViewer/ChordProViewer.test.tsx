...
Some lyrics`;
    ...
    it('handles metadata with colons in values', () => {
      ...
      expect(
        screen.getByText('{subtitle: A Song About: Love and Hope}')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Content Parsing', () => {
    it('renders simple lyrics', () => {
      ...
      expect(screen.getByText('Here are some simple lyrics')).toHaveClass(
        'chordpro-line',
        'chordpro-lyrics'
      );
      expect(screen.getByText('On multiple lines')).toHaveClass(
        'chordpro-line',
        'chordpro-lyrics'
      );
    });
    ...
    it('renders directives', () => {
      ...
      expect(screen.getByText('{chorus}')).toHaveClass(
        'chordpro-line',
        'chordpro-directive'
      );
      expect(screen.getByText('{repeat}')).toHaveClass(
        'chordpro-line',
        'chordpro-directive'
      );
    });
...
{end_of_grid}`;
  ...
  describe('Edge Cases', () => {
    it('handles malformed chord brackets', () => {
      ...
      expect(
        screen.getByText('Another ]only closing bracket')
      ).toBeInTheDocument();
    });
    ...
    it('handles malformed directive braces', () => {
      ...
      expect(
        screen.getByText('Another }only closing brace')
      ).toBeInTheDocument();
    });
    ...
    it('handles special characters', () => {
      ...
      expect(
        screen.getByText('Special chars: é, ñ, 中文, 🎵, & < > "quotes"')
      ).toBeInTheDocument();
    });
...
More regular lyrics`;
      ...
      expect(screen.getByText('# Comment line')).toHaveClass(
        'chordpro-comment'
      );
      ...
      const sectionHeader = sectionHeaders.find((el) =>
        el.classList.contains('chordpro-section-header')
      );
