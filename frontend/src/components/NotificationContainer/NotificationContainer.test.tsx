...
vi.mock('../NotificationToast', () => ({
  default: ({ notification, onClose }: { notification: SharingNotification; onClose: () => void }) => (
...
describe('NotificationContainer', () => {
  ...
  it('renders all notifications', () => {
    ...
    expect(screen.getByTestId('notification-notification-2')).toBeInTheDocument();
    expect(screen.getByTestId('notification-notification-3')).toBeInTheDocument();
  ...
  it('calls onRemoveNotification when notification close button is clicked', () => {
    ...
    render(<NotificationContainer {...defaultProps} onRemoveNotification={onRemoveNotification} />);
  ...
  it('renders notifications in the order they are provided', () => {
    ...
    render(<NotificationContainer notifications={reversedNotifications} onRemoveNotification={vi.fn()} />);
  ...
  it('handles single notification correctly', () => {
    ...
    render(<NotificationContainer notifications={singleNotification} onRemoveNotification={vi.fn()} />);
    
    expect(screen.getByTestId('notification-notification-1')).toBeInTheDocument();
    expect(screen.queryByTestId('notification-notification-2')).not.toBeInTheDocument();
    expect(screen.queryByTestId('notification-notification-3')).not.toBeInTheDocument();
  });
  ...
  it('updates when notifications prop changes', () => {
    ...
    rerender(<NotificationContainer notifications={updatedNotifications} onRemoveNotification={vi.fn()} />);
  ...
  it('handles notifications with duplicate IDs gracefully', () => {
    ...
    render(<NotificationContainer notifications={duplicateNotifications} onRemoveNotification={vi.fn()} />);
