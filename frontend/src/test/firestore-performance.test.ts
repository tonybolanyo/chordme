...
describe('Firestore Real-time Update Throughput - Performance Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.warn('Firebase emulator not available. Performance tests will be skipped.');
  ...
  const calculateMetrics = (
    ...
    const updateLatencies = updateTimes.map((time, index) => 
      index === 0 ? 0 : time - updateTimes[index - 1]
    ).slice(1);
    
    const listenerLatencies = listenerTimes.map((listenerTime, index) => {
  ...
  describe('Single Document Update Performance', () => {
    it('should handle rapid sequential updates within performance thresholds', async () => {
      ...
      for (let i = 1; i <= numberOfUpdates; i++) {
        ...
        await assertSucceeds(updateDoc(songRef, {
      ...
      expect(metrics.averageLatency).toBeLessThan(PERFORMANCE_THRESHOLDS.MAX_AVERAGE_LATENCY);
      expect(metrics.maxLatency).toBeLessThan(PERFORMANCE_THRESHOLDS.MAX_SINGLE_UPDATE_LATENCY);
      expect(metrics.throughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
    ...
    it('should handle burst updates with acceptable latency', async () => {
      ...
      for (let i = 1; i <= numberOfUpdates; i++) {
        updatePromises.push(
          assertSucceeds(updateDoc(songRef, {
      ...
      expect(burstThroughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
    ...
    it('should maintain performance under sustained load', async () => {
      ...
      expect(actualThroughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
  ...
  describe('Collection Update Performance', () => {
    it('should handle multiple document updates efficiently', async () => {
      ...
      for (let i = 1; i <= numberOfDocs; i++) {
        ...
        await assertSucceeds(setDoc(doc(db, 'songs', docId), {
      ...
      for (const docId of docIds) {
        updatePromises.push(
          assertSucceeds(updateDoc(doc(db, 'songs', docId), {
            counter: 1,
      ...
      // Performance assertions
      expect(updateDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.MAX_BATCH_TIME);
      expect(collectionThroughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
    ...
    it('should handle batch operations efficiently', async () => {
      ...
      const querySnapshot = await assertSucceeds(context.firestore().getDocs(collectionQuery));
      ...
      expect(batchThroughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
  ...
  describe('Stress Testing', () => {
    it('should handle high-volume updates without degradation', async () => {
      ...
      const lastWindowThroughput = performanceWindows[performanceWindows.length - 1].throughput;
      const performanceDegradation = (firstWindowThroughput - lastWindowThroughput) / firstWindowThroughput;
      
      // Performance assertions
      expect(performanceDegradation).toBeLessThan(0.5); // Less than 50% degradation
      expect(lastWindowThroughput).toBeGreaterThan(PERFORMANCE_THRESHOLDS.MIN_THROUGHPUT);
  ...
  describe('Latency Measurements', () => {
    it('should measure end-to-end latency for real-time updates', async () => {
      ...
      for (let i = 1; i <= numberOfLatencyTests; i++) {
        ...
        await assertSucceeds(updateDoc(songRef1, {
      ...
      if (validLatencies.length > 0) {
        ...
        const medianLatency = validLatencies.sort((a, b) => a - b)[Math.floor(validLatencies.length / 2)];
        
        // Latency assertions
        expect(averageLatency).toBeLessThan(PERFORMANCE_THRESHOLDS.MAX_AVERAGE_LATENCY);
        expect(maxLatency).toBeLessThan(PERFORMANCE_THRESHOLDS.MAX_SINGLE_UPDATE_LATENCY);
