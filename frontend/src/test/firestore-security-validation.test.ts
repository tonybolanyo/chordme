...
describe('Firestore Security Rules Validation', () => {
  ...
  it('should have necessary indexes for queries', () => {
    ...
    const songsIndexes = indexes.indexes.filter((index: any) => 
      index.collectionGroup === 'songs'
    );
    expect(songsIndexes.length).toBeGreaterThan(0);
    
    // Check for chords indexes
    const chordsIndexes = indexes.indexes.filter((index: any) => 
      index.collectionGroup === 'chords'
    );
  ...
  it('should prevent changing ownership fields', () => {
    ...
    expect(rulesContent).toContain('request.resource.data.author_id == resource.data.author_id');
    expect(rulesContent).toContain('request.resource.data.created_at == resource.data.created_at');
  });
