...
describe('Firestore Real-time Sync Behavior - Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.warn('Firebase emulator not available. Real-time sync tests will be skipped.');
  ...
  describe('Single Document Real-time Updates', () => {
    ...
    it('should handle rapid successive updates', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        updatePromises.push(
          assertSucceeds(updateDoc(songRef1, {
            counter: i,
            updated_at: Timestamp.now(),
          }))
        );
  ...
  describe('Collection Real-time Updates', () => {
    ...
    it('should receive updates when documents are modified in collection', async () => {
      ...
      await assertSucceeds(setDoc(doc(db1, 'songs', 'modify-song-1'), song1Data));
      await assertSucceeds(setDoc(doc(db1, 'songs', 'modify-song-2'), song2Data));
      ...
      // Modify songs
      await assertSucceeds(updateDoc(doc(db1, 'songs', 'modify-song-1'), {
      ...
      await assertSucceeds(updateDoc(doc(db1, 'songs', 'modify-song-2'), {
        status: 'published',
        updated_at: Timestamp.now(),
      }));
      ...
      const updatedSong1 = finalUpdate.find((s: any) => s.id === 'modify-song-1');
      const updatedSong2 = finalUpdate.find((s: any) => s.id === 'modify-song-2');
    ...
    it('should handle document removal from collection', async () => {
      ...
      // Create initial songs
      await assertSucceeds(setDoc(doc(db1, 'songs', 'remove-song-1'), {
        title: 'Song to Keep',
        user_id: USER_1_ID,
        created_at: Timestamp.now(),
      }));
      
      await assertSucceeds(setDoc(doc(db1, 'songs', 'remove-song-2'), {
      ...
      expect(removalUpdates[removalUpdates.length - 1].titles).toContain('Song to Keep');
      expect(removalUpdates[removalUpdates.length - 1].titles).not.toContain('Song to Remove');
    });
  });
  ...
  describe('Multi-client Concurrent Editing', () => {
    ...
    it('should handle conflicting updates with last-write-wins', async () => {
      ...
      const snapshot1 = await assertSucceeds(context1.firestore().doc('songs', songId).get());
      const snapshot2 = await assertSucceeds(context2.firestore().doc('songs', songId).get());
  ...
  describe('Real-time Performance', () => {
    it('should handle high-frequency updates efficiently', async () => {
      ...
      const finalSnapshot = await assertSucceeds(context2.firestore().doc('songs', songId).get());
